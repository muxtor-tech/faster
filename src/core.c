...
size_t faster_mb_to_unicode(const char* src, fchar_t* _dest, size_t dest_size) {
    if (!src || !_dest || dest_size == 0) return 0;
...
fchar_t* faster_strdup(const fchar_t* s) {
    ...
    if(dup) memcpy(dup, s, (len + 1) * sizeof(fchar_t));
    return dup;
}
...
size_t faster_get_optimal_block_size(const size_t element_size, const size_t initial_count) {
    ...
    // Round up to the nearest multiple of 16 bytes (minimum allocation on many systems)
...
faster_str_t faster_str_create(const fchar_t *null_terminated_str) {
    const faster_str_t str = {
        .str_ptr = (fchar_t* const)null_terminated_str,
        .str_len = faster_strlen(null_terminated_str)
    };
    return str;
}
#pragma GCC diagnostic pop