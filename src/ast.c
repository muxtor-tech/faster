...
faster_error_code_t faster_ast_init(faster_ast_ptr_t ast) {
  ...
  faster_ast_runtime_state_t runtime_state = {.call_stack_depth = 0,
                                              .return_depth = 0,
                                              .loop_depth = 0,
                                              .loop_exit = 0,
                                              .loop_continue = false,
...
faster_operator_definition_t faster_operator_list[] = {{.operator_str = {.str_ptr = _faster_operator_str_add, .str_len = 1},
...
static inline faster_operator_definition_t *_get_operator_definition_with_m(const fchar_t *str,
                                                                            const faster_system_indexing_t str_len, bool maybe) {
  for (int i = 0; faster_operator_list[i].operator_str.str_ptr != NULL; i++) {
    if ((maybe ? (faster_operator_list[i].operator_str.str_len >= str_len)
               : (faster_operator_list[i].operator_str.str_len == str_len)) &&
...
static inline faster_operator_associativity_t _get_operator_associativity(const fchar_t *str,
                                                                          const faster_system_indexing_t str_len) {
  faster_operator_definition_t *op_def = _get_operator_definition(str, str_len);
  return op_def ? op_def->operator_associativity : FAST_OPERATOR_ASSOCIATIVITY_UNDEFINED;
}

static inline faster_operator_function_base_t _get_operator_function_base(const fchar_t *str,
                                                                          const faster_system_indexing_t str_len) {
  faster_operator_definition_t *op_def = _get_operator_definition(str, str_len);
  return op_def ? op_def->operator_function_base : FAST_OPERATOR_FUNCTION_BASE_UNDEFINED;
}

static inline faster_operator_function_definition_t _get_operator_function(const fchar_t *str,
                                                                           const faster_system_indexing_t str_len) {
  faster_operator_definition_t *op_def = _get_operator_definition(str, str_len);
...
faster_error_code_t faster_tokenize(faster_ast_ptr_t ast, const faster_str_ptr_t str) {
  ...
  while (token_current_pos < str_len) {
    ...
    case FAST_TOKENIZER_STATE_SYMBOL:
      ...
      //     new_token.token_str.str_len = token_current_pos -
      //     token_starting_pos; faster_token_t_arr_set(&ast->token_list,
      //     token_id++, &new_token); state = FAST_TOKENIZER_STATE_FLAT;
      // }
      break;
  ...
  if (state == FAST_TOKENIZER_STATE_OPERATOR || state == FAST_TOKENIZER_STATE_NUMBER) {
    // new_token.token_type = (state == FAST_TOKENIZER_STATE_OPERATOR) ?
    // FAST_TOKEN_TYPE_OPERATOR : FAST_TOKEN_TYPE_NUMBER;
    // new_token.token_str.str_ptr = &str_ptr[token_starting_pos];
