...
static int height(const AVLNodesTreePtr tree, const AVLNodeIndex node) { return FASTER_AVL_NODE_VALID(node) ? tree->node_list.list[node].height : 0; }
...
// insert wrapper
static AVLNodeIndex _AVL_insert(const AVLNodesTreePtr tree, const AVLNodeIndex node, const faster_str_ptr_t key, const faster_value_ptr value, bool * found);
...
static AVLNodeIndex _AVL_insert(const AVLNodesTreePtr tree, const AVLNodeIndex node, const faster_str_ptr_t key, const faster_value_ptr value, bool * found) {
    ...
    tree->node_list.list[node].height = max(height(tree, tree->node_list.list[node].left), height(tree, tree->node_list.list[node].right)) + 1;
...
faster_value_ptr AVL_get(const AVLNodesTreePtr tree, const faster_str_ptr_t key) {
    ...
    while (FASTER_AVL_NODE_VALID(node)) {
        ...
        if (cmp == 0) return node_ptr->value; // key found
...
static AVLNodeIndex _AVL_remove(const AVLNodesTreePtr tree, const AVLNodeIndex node, const faster_str_ptr_t key, bool * found) {
    if (FASTER_AVL_NODE_INVALID(node)) return node;
    AVLNodeIndex index = node;
    ...
    if (FASTER_AVL_NODE_INVALID(index)) return index;
