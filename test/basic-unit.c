...
void test_invalid_sequences() {
    ...
    assert(unicode_buf[2] == 'c');
        ...
        // In one-byte mode, invalid bytes become - EF BF BD - � (replacement character)
    ...
    #else
        // In two-byte and four-byte modes, invalid bytes become U+FFFD - � (replacement character)
        assert(conv == 5);
