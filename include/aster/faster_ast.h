...
// typedef a function pointer to a function that takes an index of a ast_node and the ast structure and returns a pointer
...
// we will have less nodes than tokens, but we will need more nodes during actual execution
// we use the AVL tree structure to store the context of the AST
// but runtime status will be kept directly for speed
// ast is actually a tree of nodes, but we use a flat array to store the nodes
...
#define DECLARE_AST_WITH_DYNAMIC_ALLOCATION(name, token_capacity) \
    ...
    faster_ast_t name = { \
        .token_list = name##_token_array, \
        .ast_list = name##_ast_array, \
        .context_tree = name##_faster_ast_context_tree, \
        .ast_root_id = FASTER_ARRAY_COUNT_INVALID \
        .float_list = name##_float_array \
        .runtime_state = { 0 } \
    }
...
#define FASTER_VALUE_STORE_FLOAT(ast, float_value) (faster_value_ptr)(((faster_value_ptr_handler_t)ast->float_list.list + faster_value_float_holder_t_arr_get_next(&ast->float_list)) | FASTER_VALUE_MARKER_FLO)
